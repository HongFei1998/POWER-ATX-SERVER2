...
from javacap import start_javacap_thread
...
class AndroidDevice(object):
    ...
    async def init(self):
        ...
        self.javacap_address = start_javacap_thread(self._serial)
    ...
    def _init_binaries(self):
        ...
        # 如果是小米则不推送，需要手动选择MXBC_ATX\Xiaomi_Vector_issue-main合适的minicap.so文件放入手机 /data/local/tmp，后续如果有需求可改为识别型号放对应的minicap.so
        device_info = subprocess.getoutput(f'adb -s {self._serial} shell "getprop | grep product"').lower()
        if 'xiaomi' in device_info:
            pass
        else:
    ...
    def addrs(self):
        ...
        return {
            ...
            "javacapAddress": self.javacap_address,
        }
